...
public class BranchDomainService implements IBranchDomainService {
    ...
    public BranchDomainService(BranchRepository branchRepository, IMapper mapper,
    ...
    public CompletableFuture<BranchDto> getById(int orgId, int id) {
        ...
        return CompletableFuture
                .supplyAsync(
                        () -> branchRepository.findById(id).orElseThrow(() -> new RuntimeException("BranchNotFound")))
    ...
    public CompletableFuture<BranchDto> create(int orgId, BranchDto branchDto) {
        return CompletableFuture
                .supplyAsync(() -> organizationRepository.findById(orgId)
                        .orElseThrow(() -> new RuntimeException("OrganizationNotFound")))
                .thenApply(org -> this.createBranch(branchDto, org)).thenApply(b -> mapper.toBranchDto(b));
    ...
    public CompletableFuture<BranchDto> update(int orgId, BranchDto branchDto) {
        return CompletableFuture
                .supplyAsync(() -> branchRepository.findByIdAndOrganizationId(branchDto.getId(), orgId)
                        .orElseThrow(() -> new RuntimeException("BranchNotFound")))
                .thenApply(branch -> updateBranch(branch, branchDto)).thenApply(branch -> mapper.toBranchDto(branch));
